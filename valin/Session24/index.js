...
demoBoxElement.className = ''; // adauga o clasa dar le sterge pe celelalte

demoBoxElement.classList.add('blue'); // adauga o clasa la celelalte
demoBoxElement.classList.add('red');
demoBoxElement.classList.remove('blue');
demoBoxElement.classList.toggle('blue'); // adauga clasa daca nu este sau o scoate daca deja este
...
function toggleListItems() {
   // console.log(this.classList.toggle('red')) // toggle ne returneaza daca elementul are clasa
...
// addButtonElement.addEventListener('click', function() { // semplificat cat mai mult functia asta, practic sa apelam functii externe
//    const canAdd = checkPlan();                          // pentru a face acelasi lucru si sa stylam in css
...
function checkPlan() {
   const newListItemsNumber = document.getElementsByClassName('newLiElement').length;
...
function addListElement() {
   newLiElement.innerHTML = toDoInputElement.value;
   toDoListElement.appendChild(newLiElement);
   toDoInputElement.value = '';
   toDoInputElement.focus(); // focus dupa ce dam click si trimitem valoarea
}
